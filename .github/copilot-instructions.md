...
| `curator-review-summary.json` | Aggregated count of current, non-hidden curator identifications per species, derived from `raw-inat-data/`. Shape: `{ generatedAt: string, fileCount: number, counts: Record<taxonId, number> }`. This is the single source of truth for curator review counts covering all species, not just those on the baseline.  |
...
| `inat-data-log.json`          | Metadata about the last iNat data download: `{ lastRun, totalObservations, totalPackets, durationSeconds, success, error? }`                                                                                                                                                                                          |
| `raw-inat-data/packet-N.json` | Raw paginated iNaturalist API responses downloaded during the iNat data download step. Consumed by checklist generation and curator summary generation.                                                                                                                                                               |
